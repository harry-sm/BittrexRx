...
export class CloudflareAuthenticator {
	...
	// 	 AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2859.0 Safari / 537.36`;
	// }

	private cloudFlareRequest() {
		if (!this.cfCredentials) {
			const cfResponse: Observable<any> = Observable.create((observer: Subscriber<any>) => {
				cloudscraper.get('https://bittrex.com/', function(error: any, response: any) {
					if (error) {
